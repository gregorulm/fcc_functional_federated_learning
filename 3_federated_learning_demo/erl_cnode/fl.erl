...
server(
  Client_Pids_Nums, Data, Data_List, Model_Ws, N, StartTime, Stats) ->
  ...
  lists:map(
    fun({Sname, Num}) ->
      call_cnode(
        self(), Sname, {assignment, Model_Ws, maps:get(Num, Data)}) end,
      Subset),
  ...
  Sum_Local_Ws = lists:map(
    fun(L) -> fold_lists(tl(L), hd(L)) end, [W_In, W_Bias, W_Out] ),
    ...
    false ->
      case N rem 500 == 0 of
        ...
        true -> io:format("N: ~p~n~n", [N]),
                ...
                case os:system_time(seconds) - StartTime >= 60*10 of
                  true  -> io:format("Stop! N: ~p~n", [N]),
                           Stats_;
                  false -> server(Client_Pids_Nums, Data, Data_List,
...
call_client_ann(Id) ->
  ...
  os:cmd("x-terminal-emulator -e ./client_ann "
         ++ ServerNode ++ " " ++ StrId),
...
main() ->
  ...
  write_results("data/output_erl_cnodes"
